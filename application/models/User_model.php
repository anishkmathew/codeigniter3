...
Class User_model extends CI_Model
{
	...
	public function login()
	{
		...
		$table = $this->db->dbprefix('users');
 		$sql = "SELECT * FROM $table WHERE password = '$pass_encrypt' AND email = ".$this->db->escape($this->input->post('email'))." LIMIT 1";
		$where	=	array(
	...
	public function get_users($order_by = 'id', $order = 'asc', $limit = FALSE, $start = 0)
	{
		$this->db->select('*');
		$this->db->from('users');
	...
	*/
	public function get_users_pagination_view($order_by = 'id', $order = 'asc', $limit = 0, $start = 0){
		$output	=	'';
		$user_data = $this->get_users('id', 'asc', $limit, $start);
		 ...
		 foreach($user_data as $row)
		  {
		   ...
		   <tr>
			...
			<td>'.$row['username'].'</td>
			<td>'.$row['email'].'</td>
			<td><span class="badge bg-red">'.$row['id'].'</span></td>
		   </tr>
		   ';
		  }
		  $output .= '</table>';
	...
	public function set_user($image = FALSE)
	{
